										...
										<td >{{ (int)$incidentType->activo === 1 ? 'Sí' : 'No' }}</td>
                                            ...
                                            <td>
                                                <form action="{{ route('incident-types.destroy', $incidentType->id) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('incident-types.show', $incidentType->id) }}"><i class="fa fa-fw fa-eye"></i> <span class="d-none d-md-inline">{{ __('Ver') }}</span></a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('incident-types.edit', $incidentType->id) }}"><i class="fa fa-fw fa-edit"></i> <span class="d-none d-md-inline">{{ __('Editar') }}</span></a>
                                                    @csrf
                                                    @method('DELETE')
